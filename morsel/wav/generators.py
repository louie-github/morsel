...
def generate_sine_wave_file(
    ...
    *,
    ...
    of *buffer_size* bytes before writing them into the file.

    Note, however, that *buffer_size* is only a target, and will
    most likely not be achieved because the inner iterator may return
    bytes objects that are larger than the specified buffer size, or
    bytes objects that do not cleanly add up to the specified buffer
    size.
    ...
    with open(filename, "wb") as f:
        ...
        if cycles_per_buffer <= 1:
            ...
            bytes_written += f.write(first_cycle_data)
            for cycle_data in data:
                bytes_written += f.write(cycle_data)
        else:
